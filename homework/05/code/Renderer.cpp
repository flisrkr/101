...
// orig: ray origin
// dir: ray direction
// objects: list of objects the scene contains
// tNear: contains the distance to the closest intersected object
// index: stores the index of the intersect triangle if the intersected object is a mesh
// uv: stores the u and v barycentric coordinates of the intersected point
// hitObject: stores the pointer to the intersected object (used to retrieve material information, etc.)
...
//
// This function computes the color at the intersection point of a ray defined by a position and a direction.
// Note that this function is recursive (it calls itself).
//
...
void Renderer::Render(const Scene &scene)
{
    ...
    for (int j = 0; j < scene.height; ++j)
    {
        for (int i = 0; i < scene.width; ++i)
        {
            ...
            x = ((2 * static_cast<float>(i) + 1) / static_cast<float>(scene.width) - 1) * scale * imageAspectRatio;
            y = (1 - (2 * static_cast<float>(j) + 1) / static_cast<float>(scene.height)) * scale;
